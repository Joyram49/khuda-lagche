...
import { editFoodById } from "@/app/actions/foodItems";
...
function EditFoodItemModal({ onClose, foodInfo, restaurantInfo, categories }) {
  const [error, setError] = useState("");
  const [foodImagePath, setFoodImagePath] = useState(foodInfo?.image_url ?? "");
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      delivery_time: parseInt(foodInfo?.delivery_time) || 30,
      preparation_time: parseInt(foodInfo?.preparation_time) || 15,
      price: parseInt(foodInfo?.price) || 0,
      quantity: parseInt(foodInfo?.quantity) || 1,
      category: foodInfo?.category?.id || "",
      tags: foodInfo?.tags.join(", ") || "",
    },
  });
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await editFoodById({
        foodId: foodInfo?.id,
        requestData,
      });
      if (response?.status === 200) {
        toast.success(`${foodInfo?.name} successfull updated`);
        form.reset();
        onClose();
      }
    } catch (error) {
      toast.error(`${foodInfo?.name} failed to update`);
